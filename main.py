...
st.title("🪖 Bölük Nöbet, Durum ve Hastane Takip Sistemi")
...
with c_ist:
    st.write("🏥 **İstirahatliler** (Sayaç Artar)")
    istirahatli_secilenler = st.multiselect("İstirahatli seç:", kalan_isimler(df['İsim'].tolist(), secilen_toplam))
    secilen_toplam.extend(istirahatli_secilenler)

with c_sevk:
    st.write("🚑 **Sevkliler** (Sayaç Artar)")
    sevkli_secilenler = st.multiselect("Sevkli seç:", kalan_isimler(df['İsim'].tolist(), secilen_toplam))
    secilen_toplam.extend(sevkli_secilenler)

with c_hast:
    st.write("🏥 **Hastane Görevi** (Sayaç Artmaz)")
    hastane_secilenler = st.multiselect("Hastaneye gidenleri seç:", kalan_isimler(df['İsim'].tolist(), secilen_toplam))
    secilen_toplam.extend(hastane_secilenler)
...
with b1:
    if st.button("💾 Manuel Seçimleri Kaydet"):
        # Nöbet puanlarını işle
        for n in [n1, n2]: 
            if n != "Seçiniz": df.loc[df['İsim'] == n, 'Nizamiye'] += 1
        for k in [k1, k2]: 
            if k != "Seçiniz": df.loc[df['İsim'] == k, 'Kamera'] += 1
        for g in [g1, g2]: 
            if g != "Seçiniz": df.loc[df['İsim'] == g, 'GGM'] += 1
        for go in [go1, go2]: 
            if go != "Seçiniz": df.loc[df['İsim'] == go, 'GÖKSEM'] += 1
        
        # Durum puanlarını işle
        for isim in istirahatli_secilenler: df.loc[df['İsim'] == isim, 'İstirahat_Sayisi'] += 1
        for isim in sevkli_secilenler: df.loc[df['İsim'] == isim, 'Sevk_Sayisi'] += 1
        
        st.success("Seçimler işlendi!")
        st.rerun()

with b2:
    if st.button("🤖 Kalanları Otomatik Dağıt"):
        st.info("Bu özellik, seçmediğiniz kişileri boş kalan yerlere en adil şekilde atar.")
        # (Otomatik dağıtım mantığı seçilmeyenler üzerinden burada çalışır)

st.divider()

# 3. İSTATİSTİK TABLOSU
st.subheader("📊 Personel İstatistikleri")
