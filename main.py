...
# 1. BÖLÜM: PERSONEL LİSTESİ VE HAFIZA
...
# 2. BÖLÜM: SEÇİM MANTIĞI VE FİLTRELEME
...
if st.button("💾 Tüm Çizelgeyi Kaydet ve Sayıları Güncelle"):
    # Nöbetleri işle
    for k in [n1, n2]: 
        if k != "Seçiniz": df.loc[df['İsim'] == k, 'Nizamiye'] += 1
    for k in [k1, k2]: 
        if k != "Seçiniz": df.loc[df['İsim'] == k, 'Kamera'] += 1
    for k in [g1, g2]: 
        if k != "Seçiniz": df.loc[df['İsim'] == g1, 'GGM'] += 1
